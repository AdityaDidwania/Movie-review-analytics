...
#View(movie_metadata)
#omit_df <- na.omit(movie_metadata)
#str(omit_df)
#sum(duplicated(omit_df))
#omit_df_no_duplicate <- omit_df[!duplicated(omit_df),]
#str(omit_df_no_duplicate)
#head(omit_df_no_duplicate$genres)

#hist(omit_df_no_duplicate$gross/10^8)
...
summary(reg1)
```
# Checking for final_df correlation, (Trying using PCA for modeling, yet it doesn't seem to work!!!) 
# (Ignore this !!!!!)
...
pca2 <- preProcess(final_df, method = c("center", "scale", "pca"))
nor <- preProcess(final_df[,c(-11,-12,-39)], method = c("center", "scale"))

final_df_norm <- predict(nor, final_df[,c(-11,-12,-39)])
final_df_pca <- predict(pca2, final_df[,c(-11,-12)])
pca1 <- prcomp(final_df_norm)
biplot(pca1)
summary(pca1)
...
train.df <- final_df_pca[train.index,]
valid.df <- final_df_pca[-train.index,]

library(rpart)
library(rpart.plot)
class.tree <- rpart(gross_catogorical ~. , data = train.df, method = "class")
...
#prune tree
set.seed(23)
crossvalid_ct <- rpart(gross_catogorical ~., data = train.df, method = "class", cp = 0.001, minsplit = 5, xval = 5)
printcp(crossvalid_ct)

pruned_ct <- prune(crossvalid_ct, 
                   cp = crossvalid_ct$cptable[which.min(crossvalid_ct$cptable[,"xerror"]),"CP"])
...
for(i in 1:30) {
  knn.pred <- knn(train.df[, -1], valid.df[, -1],
                  cl = train.df[, 1], k = i)
  accuracy.df[i, 2] <- confusionMatrix(knn.pred, valid.df[, 1])$overall[1]
}
accuracy.df
...
Accuracy for validation dataset is 0.6929
...
rf <- randomForest(gross_catogorical ~ . -gross -`Sci-Fi`, data = train.df, mtry = 5)
# Show model error
plot(rf)
...
# Try classification tree method to predict profitable (Somehow I can't get it to work)
```{r}
sum(is.na(train.df.logistic))
class.tree.profit <- rpart(profitable ~. -gross - gross_catogorical, data = train.df.logistic, method = "class")
prp(class.tree.profit, type = 1, extra = "auto", split.font = 1, varlen = 0)


class.tree.profit.pred <- predict(class.tree.profit, valid.df.logistic, type = "class")
#accuracy(class.tree, valid.df$gross)


confusionMatrix(class.tree.profit.pred, as.factor(valid.df.logistic$profitable))
```
